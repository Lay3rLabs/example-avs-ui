import { TestnetConfig } from "@/utils/constants";
 ...
 * @param denom - The denomination to check the balance of (e.g., "uperm" for the SLAY token).
 * @returns A promise that resolves to a balance object containing the amount and denomination.
 *
 * @example
 * const balance = await fetchUserBalance("layer1address...", "uperm");
 * console.log(balance); // { amount: "1000000", denom: "uperm" }
 */
export const fetchUserBalance = async (address: string, denom: string) => {
  ...
  const client = await StargateClient.connect(TestnetConfig.rpc_endpoint);
